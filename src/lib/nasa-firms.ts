...
export function confidenceToIntensity(
...
export async function fetchFIRMSData(days: number = 1): Promise<FIRMSHotspot[]> {
  ...
  console.log(
    `[FIRMS] Parsed ${hotspots.length} valid hotspots from ${csv.split('\n').length - 1} rows`,
  )
...
export function isDuplicate(
  ...
  return existingReports.some(
    (report) =>
      haversineDistance(hotspot.latitude, hotspot.longitude, report.latitude, report.longitude) <
      thresholdKm,
  )
}
